    ...
    <section id="section-action-items" data-testid="editor-action-items">
      ...
      <div className="space-y-4">
        {actionItems.map((item) => (
          <Card key={item.id} className={`p-3 border-l-4 ${item.type === "immediate" ? "border-l-amber-400" : "border-l-blue-400"}`} data-testid={`action-item-block-${item.id}`}>
      ...
      {actionItems.length > 0 && (
        <div className="mt-4 flex items-center gap-4 text-xs text-muted-foreground">
          <span className="flex items-center gap-1"><Zap className="w-3 h-3 text-amber-500" /> {immediateItems.length} immediate</span>
